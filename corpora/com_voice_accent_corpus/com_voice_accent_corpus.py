...
class ComVoiceAccentCorpusConfig(datasets.BuilderConfig):
    ...
    def __init__(self, **kwargs):
        ...
        """
        super(ComVoiceAccentCorpusConfig, self).__init__(version=datasets.Version("2.1.0", ""), **kwargs)
...
class ComVoiceAccentCorpus(datasets.GeneratorBasedBuilder):
    """DialectSpeechCorpus dataset."""

    BUILDER_CONFIGS = [
        ComVoiceAccentCorpusConfig(name="clean", description="'Clean' speech."),
    ]
    ...
    def _generate_examples(self, archive_path):
        ...
        for _, c in enumerate(os.listdir(wav_dir)):
            if os.path.isdir(f'{wav_dir}/{c}/'):
                for file in os.listdir(f'{wav_dir}/{c}/')[:3300]:
